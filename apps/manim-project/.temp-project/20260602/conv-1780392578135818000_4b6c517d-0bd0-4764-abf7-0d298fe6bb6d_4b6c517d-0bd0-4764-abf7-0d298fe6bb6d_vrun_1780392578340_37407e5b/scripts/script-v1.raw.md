...
from manim import (
    ...
    YELLOW, BLUE, WHITE, MathTex, Text, Ellipse, Line, Arrow,
    Polygon, FadeOut, MoveToTarget, Rotating, ValueTracker,
    UpdateFromAlpha, always_redraw, Transform
)
...
class KeplerLawsScene(BaseScene):
    ...
    def _page_1(self, total_pages: int) -> None:
        ...
        f1_label = MathTex("F_1", font_size=36).next_to(f1, DOWN)
        f2_label = MathTex("F_2", font_size=36).next_to(f2, DOWN)
    ...
    def _page_3(self, total_pages: int) -> None:
        ...
        a1_label = MathTex("a_1", font_size=32, color=BLUE).next_to(a1_line, DOWN)
        a2_label = MathTex("a_2", font_size=32, color=GREEN).next_to(a2_line, DOWN)
