...
from manim import (
    Circle, RoundedRectangle, Line, Arrow, Text, VGroup, FadeIn, FadeOut, Create,
    GrowArrow, Indicate, Transform, ApplyMethod, RED, BLUE, YELLOW, WHITE, BLACK,
    ORIGIN, UP, DOWN, LEFT, RIGHT, Wait
)
...
class ElectrolysisOfWaterScene(BaseScene):
    ...
    def _page_2(self, total_pages: int) -> None:
        ...
        # 化学方程式
        equation = create_chinese_formula(r"2H_2O \stackrel{\text{通电}}{=\!=\!=} 2H_2\uparrow + O_2\uparrow", font_size=42).shift(DOWN * 2.8)
        self.wait(1.0)
    ...
    def _page_3(self, total_pages: int) -> None:
        ...
        # 缩小的化学方程式
        small_equation = create_chinese_formula(r"2H_2O \stackrel{\text{通电}}{=\!=\!=} 2H_2\uparrow + O_2\uparrow", font_size=28).shift(UP * 2.8)
