...
from manim import (
    ...
    Succession, AnimationGroup, ScaleInPlace, FadeOut, ApplyMethod, UP, DOWN, LEFT, RIGHT,
...
class ElectrolysisOfWaterScene(BaseScene):
    ...
    def _page_1(self, total_pages: int) -> None:
        ...
        self.add_animation(right_title, animation=FadeIn(right_title), side="right", run_time=1)
        self.wait(1)

        right_desc = self.add_text("实验条件：直流电 水加少量电解质增强导电性", font_size=30, side="right", animate=False)
        right_desc.shift(DOWN * 2)
        self.add_animation(right_desc, animation=ApplyMethod(right_desc.shift, UP * 2), side="right", run_time=2)
    ...
    def _page_2(self, total_pages: int) -> None:
        ...
        # 左上角文字
        title_before = Text("通电前：水分子无序运动", font_size=28)
        title_before.to_corner(UL)

        # 飞入动画
        fly_anims = [FadeIn(mol) for mol in water_molecules]
        fly_anims.append(FadeIn(title_before))
        self.add_animation(water_molecules + title_before, animation=AnimationGroup(*fly_anims, run_time=3), run_time=3)
        ...
        self.add_animation(water_molecules, animation=AnimationGroup(*move_anims, run_time=5), run_time=5)
    ...
    def _page_3(self, total_pages: int) -> None:
        ...
        self.add_animation(left_title, animation=FadeIn(left_title), side="left", run_time=1)
        ...
        self.add_animation(VGroup(pos_mol, o2), animation=Succession(
            ...
            run_time=3
        ), side="right", run_time=3)

        # 体积比2:1
        ratio_label = Text("2:1", font_size=48, color=YELLOW, weight=BOLD)
        ratio_label.to_edge(DOWN).shift(UP * 0.5)
