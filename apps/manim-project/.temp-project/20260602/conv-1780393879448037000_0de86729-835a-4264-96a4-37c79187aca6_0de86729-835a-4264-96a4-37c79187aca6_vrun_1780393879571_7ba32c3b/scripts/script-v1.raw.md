...
from manim import (
    ...
    Succession, AnimationGroup, ScaleInPlace, FadeOut, Shift, UP, DOWN, LEFT, RIGHT,
...
class ElectrolysisOfWaterScene(BaseScene):
    ...
    def _page_1(self, total_pages: int) -> None:
        ...
        right_title.set_weight(BOLD)
        self.add_animation(right_title, animation=FadeIn(right_title), run_time=1)
        self.add_animation(Wait(), run_time=1)

        right_desc = self.add_text("实验条件：直流电 水加少量电解质增强导电性", font_size=30, side=RIGHT, animate=False)
        right_desc.move_to(right_desc.get_center() + DOWN * 2)
        self.add_animation(right_desc, animation=Shift(UP * 2), run_time=2)
        ...
        # 左侧淡入，标注依次弹出
        self.add_animation(left_group, animation=FadeIn(left_group), run_time=2)
        self.add_animation(Succession(*[Create(label, run_time=0.5) for label in labels]), run_time=2.5)
    ...
    def _page_2(self, total_pages: int) -> None:
        ...
        # 飞入并无序漂浮
        self.add_animation(AnimationGroup(
            *[FadeIn(mol, shift=ORIGIN - mol.get_center()) for mol in water_molecules],
            FadeIn(title_before),
            run_time=3
        ))
        ...
        float_anims = []
        for _ in range(10):
            float_anims.append(AnimationGroup(
                *[mol.animate.shift(RIGHT * random.uniform(-0.1, 0.1) + UP * random.uniform(-0.1, 0.1)) for mol in water_molecules],
                run_time=0.3
            ))
        self.add_animation(Succession(*float_anims), run_time=3)
        ...
        self.add_animation(FadeIn(power_symbol), run_time=1)
        self.add_animation(Succession(
        ...
        self.add_animation(AnimationGroup(*move_anims), run_time=5)
    ...
    def _page_3(self, total_pages: int) -> None:
        ...
        # 左侧负极区域
        left_title = Text("负极（阴极）反应", font_size=32, color=BLUE).to_corner(UL)
        self.add_animation(FadeIn(left_title), run_time=1)
        ...
        self.add_animation(Write(neg_formula), run_time=3)
        ...
        self.add_animation(FadeIn(neg_mol), run_time=1)
        self.add_animation(Succession(
            FadeOut(neg_mol[3:]),
            FadeIn(h2),
        ...
        # 右侧正极区域
        right_title = Text("正极（阳极）反应", font_size=32, color=RED).to_corner(UR)
        self.add_animation(FadeIn(right_title), run_time=1)
        ...
        self.add_animation(Write(pos_formula), run_time=3)
        ...
        self.add_animation(FadeIn(pos_mol), run_time=1)
        self.add_animation(Succession(
            FadeOut(pos_mol[3:]),
            FadeIn(o2),
        ...
        # 体积比2:1
        ratio_label = Text("2:1", font_size=48, color=YELLOW, weight=BOLD).to_edge(DOWN).shift(UP * 0.5)
        self.add_animation(ScaleInPlace(ratio_label, scale_factor=1.5), run_time=1)
        self.add_animation(ratio_label.animate.scale(1/1.5), run_time=0.5)
    ...
    def _page_4(self, total_pages: int) -> None:
        ...
        total_formula.set_weight(BOLD)
        self.add_animation(ScaleInPlace(total_formula, scale_factor=1.3), run_time=2)
        self.add_animation(total_formula.animate.scale(1/1.3), run_time=0.5)
        self.add_animation(Wait(), run_time=1)
        ...
        for i, conc in enumerate(conclusions):
            conc.move_to(conc.get_center() + DOWN * 1)
            self.add_animation(conc.animate.shift(UP * 1), run_time=1)
            if i < 2:
                self.add_animation(Wait(), run_time=0.5)
