...
from manim import (
    ...
    Succession, AnimationGroup, ScaleInPlace, FadeOut, UP, DOWN, LEFT, RIGHT,
    ORIGIN, YELLOW, BLUE, RED, GREEN, WHITE, GRAY, LIGHT_GRAY, BLUE_C,
    Indicate, TAU, BOLD, ApplyMethod
)
...
class ElectrolysisOfWaterScene(BaseScene):
    ...
    def _page_1(self, total_pages: int) -> None:
        ...
        right_title.set(font_weight=BOLD)
        ...
        right_desc.shift(DOWN * 2)
        self.add_animation(right_desc, animation=ApplyMethod(right_desc.shift, UP * 2), run_time=2)
        ...
        # 左侧淡入，标注依次弹出
        self.add_animation(left_group, animation=FadeIn(left_group), side=LEFT, run_time=2)
        self.add_animation(labels, animation=Succession(*[Create(label, run_time=0.5) for label in labels]), side=LEFT, run_time=2.5)
    ...
    def _page_2(self, total_pages: int) -> None:
        ...
        for _ in range(12):
            ...
            mol.shift(ORIGIN + dist * np.cos(angle) * RIGHT + dist * np.sin(angle) * UP)
            water_molecules.add(mol)

        # 左上角文字
        title_before = Text("通电前：水分子无序运动", font_size=28).to_corner(UL)
        self.add_animation(title_before, animation=FadeIn(title_before), run_time=0.5)

        # 飞入并无序漂浮
        self.add_animation(water_molecules, animation=AnimationGroup(*[FadeIn(mol, shift=ORIGIN - mol.get_center()) for mol in water_molecules]), run_time=3)

        # 简单无序漂浮动画
        float_anims = []
        for _ in range(10):
            float_anims.append(AnimationGroup(*[mol.animate.shift(RIGHT * random.uniform(-0.1, 0.1) + UP * random.uniform(-0.1, 0.1)) for mol in water_molecules], run_time=0.3))
        ...
        for mol in water_molecules:
            ...
            move_anims.append(mol.animate.move_to(ORIGIN + target_x * RIGHT + target_y * UP).rotate(random.uniform(-0.5, 0.5)))
        self.add_animation(water_molecules, animation=AnimationGroup(*move_anims), run_time=5)
    ...
    def _page_3(self, total_pages: int) -> None:
        ...
        # 左侧负极区域
        left_title = self.add_text("负极（阴极）反应", font_size=32, side=LEFT, animate=False)
        left_title.set_color(BLUE)
        left_title.to_corner(UL)
        self.add_animation(left_title, animation=FadeIn(left_title), run_time=1)
        ...
        self.add_animation(neg_formula, animation=Write(neg_formula), side=LEFT, run_time=3)
        ...
        ).shift(neg_formula.get_right() + RIGHT * 1 + DOWN * 0.5)
        h2 = VGroup(
        ...
        self.add_animation(neg_mol, animation=FadeIn(neg_mol), side=LEFT, run_time=1)
        self.add_animation(VGroup(neg_mol, h2), animation=Succession(
            FadeOut(neg_mol[3:]),
            FadeIn(h2),
            h2.animate.shift(UP * 1.5),
            run_time=3
        ), side=LEFT)

        # 右侧正极区域
        right_title = self.add_text("正极（阳极）反应", font_size=32, side=RIGHT, animate=False)
        right_title.set_color(RED)
        right_title.to_corner(UR)
        self.add_animation(right_title, animation=FadeIn(right_title), run_time=1)
        ...
        self.add_animation(pos_formula, animation=Write(pos_formula), side=RIGHT, run_time=3)

        # 正极简单反应动画
        pos_mol = neg_mol.copy().shift(pos_formula.get_left() + LEFT * 1 + DOWN * 0.5)
        o2 = VGroup(
            Circle(radius=0.12, color=RED, fill_opacity=0.9),
        ...
        self.add_animation(pos_mol, animation=FadeIn(pos_mol), side=RIGHT, run_time=1)
        self.add_animation(VGroup(pos_mol, o2), animation=Succession(
            FadeOut(pos_mol[3:]),
            FadeIn(o2),
            o2.animate.shift(UP * 1.5),
            run_time=3
        ), side=RIGHT)

        # 体积比2:1
        ratio_label = Text("2:1", font_size=48, color=YELLOW, font_weight=BOLD).to_edge(DOWN).shift(UP * 0.5)
        self.add_animation(ratio_label, animation=ScaleInPlace(ratio_label, scale_factor=1.5), run_time=1)
        self.add_animation(ratio_label, animation=ratio_label.animate.scale(1/1.5), run_time=0.5)
    ...
    def _page_4(self, total_pages: int) -> None:
        ...
        self.add_animation(total_formula, animation=ScaleInPlace(total_formula, scale_factor=1.3), run_time=2)
        self.add_animation(total_formula, animation=total_formula.animate.scale(1/1.3), run_time=0.5)
        self.wait(1)

        # 三条结论
        conclusions = VGroup(
            Text("1 水由氢元素和氧元素组成", font_size=30),
            Text("2 水分子中氢原子和氧原子个数比为2:1", font_size=30),
            Text("3 化学变化中分子可分原子不可分", font_size=30)
        ).arrange(DOWN, buff=0.5, aligned_edge=LEFT).next_to(total_formula, DOWN, buff=1)

        for i, conc in enumerate(conclusions):
            conc.shift(DOWN * 1)
            self.add_animation(conc, animation=conc.animate.shift(UP * 1), run_time=1)
            if i < 2:
                self.wait(0.5)
