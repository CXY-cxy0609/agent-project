...
class ElectrolysisOfWaterScene(BaseScene):
    ...
    def _page_1(self, total_pages: int) -> None:
        ...
        # 组装装置
        apparatus_group = VGroup(container, anode, cathode, power_source, pos_label, neg_label, test_tube1, test_tube2, gas1, gas2)
        self.add_animation(apparatus_group, animation=FadeIn(apparatus_group), run_time=2)
        
        # 高亮正极和阳极
        self.add_animation(VGroup(pos_label, anode), animation=Indicate(VGroup(pos_label, anode), color=YELLOW), run_time=2)
        # 高亮负极和阴极
        self.add_animation(VGroup(neg_label, cathode), animation=Indicate(VGroup(neg_label, cathode), color=BLUE), run_time=2)
        
        # 显示体积比
        self.add_animation(ratio_label, animation=GrowFromCenter(ratio_label), run_time=1)
        self.add_animation(ratio_label, animation=Indicate(ratio_label, color=GREEN), run_time=1)
        self.add_animation(ratio_label, animation=Indicate(ratio_label, color=GREEN), run_time=1)
    ...
    def _page_2(self, total_pages: int) -> None:
        ...
        self.add_animation(VGroup(field_arrow, field_label, cathode_zone, anode_zone), animation=FadeIn(VGroup(field_arrow, field_label, cathode_zone, anode_zone)), run_time=1)
        self.add_animation(h2o_group, animation=FadeIn(h2o_group), run_time=2)
        ...
        self.add_animation(VGroup(h2_label, o2_label), animation=GrowFromCenter(VGroup(h2_label, o2_label)), run_time=2)
    ...
    def _page_3(self, total_pages: int) -> None:
        ...
        # 化学方程式（含中文文本需用create_chinese_formula）
        equation = create_chinese_formula(r"$2H_2O \xrightarrow{\text{通电}} 2H_2 \uparrow + O_2 \uparrow$", font_size=42)
        ...
        self.add_animation(equation, animation=FadeIn(equation), run_time=2)
        self.add_animation(ratio_note, animation=FadeIn(ratio_note), run_time=1)
        
        # 高亮各部分
        self.add_animation(equation, animation=Indicate(equation, color=YELLOW), run_time=2)
        self.add_animation(redox_note, animation=GrowFromCenter(redox_note), run_time=1)
        self.add_animation(redox_note, animation=Indicate(redox_note, color=GREEN), run_time=1)
        self.add_animation(redox_note, animation=Indicate(redox_note, color=GREEN), run_time=1)
    ...
    def _page_4(self, total_pages: int) -> None:
        ...
        # 高亮对应考点
        self.add_animation(VGroup(point1, diagram_group), animation=Indicate(VGroup(point1, diagram_group), color=YELLOW), run_time=2)
