...
class UserMemoryService:
    ...
    def search(self, query: str, user_id: str, top_k: int, tenant_id: str) -> list[MemorySearchResult]:
        query_vector = embedding_service.embed_one(query)
        hits = search(
            collection_name=settings.qdrant_user_memory_collection,
            query_vector=query_vector,
            top_k=top_k,
            filter_conditions={"tenant_id": tenant_id, "user_id": user_id},
        )
        return [
            MemorySearchResult(
    ...
    def store(self, user_id: str, content: str, tenant_id: str) -> None:
        ...
        upsert_chunks(
            ...
            chunks=[content],
            vectors=[vector],
            payloads=[{"tenant_id": tenant_id, "user_id": user_id}],
        )
...
class ContentCacheService:
    ...
    def search(self, query: str, top_k: int, tenant_id: str) -> list[MemorySearchResult]:
        query_vector = embedding_service.embed_one(query)
        try:
            hits = search(
    ...
    def store(self, content: str, payload: dict, tenant_id: str) -> None:
        ...
        upsert_chunks(
            ...
            chunks=[content],
            vectors=[vector],
            payloads=[{"tenant_id": tenant_id, "content_key": content, **payload}],
        )
