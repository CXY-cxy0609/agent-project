...
func (r *SQLTaskRepository) Create(ctx context.Context, task domain.Task) error {
	query := `INSERT INTO tasks (id, type, status, trace_id, created_at) VALUES (?, ?, ?, ?, ?)`
	args := []any{task.ID, task.Type, task.Status, task.TraceID, task.CreatedAt}
	if r.driver == "pgx" {
		query = `INSERT INTO tasks (id, type, status, trace_id, created_at) VALUES ($1, $2, $3, $4, $5)`
	}
...
func (r *SQLTaskRepository) GetByID(ctx context.Context, taskID string) (domain.Task, error) {
	query := `SELECT id, type, status, trace_id, created_at FROM tasks WHERE id = ?`
	arg := []any{taskID}
	if r.driver == "pgx" {
		query = `SELECT id, type, status, trace_id, created_at FROM tasks WHERE id = $1`
	}
